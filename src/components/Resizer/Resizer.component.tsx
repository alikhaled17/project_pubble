...
import { isPercentage, pxToPercent, percentToPx, getElementDimensions } from "@/core/helpers/num-to-measurement";
...
export function Resizer({ propKey, children, ...props }: any) {
  const {
    id,
    actions: { setProp },
    connectors: { connect },
  ...
  return (
    <Resizable
      ...
      onResize={(_, __, ___, d) => {
        ...
        console.log(getElementDimensions(document.querySelector(".page-container")).width, getElementDimensions(dom).width);

        if (getElementDimensions(dom).width >= getElementDimensions(document.querySelector(".page-container")).width) width = `100%`;
